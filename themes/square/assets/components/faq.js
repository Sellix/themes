    ...
    onClickItem($item) {
      ...
      if (this.activeItemId) {
        ...
        if (nestedItemIdToHide === this.activeNestedItemId) {
          ...
          $itemToHide.find('[data-faq-item-chevron] svg').css({ transform: 'rotate(0deg)', color: '' });

          $itemToHide.find('.block-faq-item__question-text').css('color', '');
        } else {
          this.activeItemId = undefined;
          this.activeNestedItemId = undefined;
        }
      ...
      if (itemId !== this.activeItemId) {
        ...
        $item.find('[data-faq-item-chevron] svg').css({ transform: 'rotate(-90deg)', color: 'var(--brandColor)' });
        $item.find('.block-faq-item__question-text').css('color', 'var(--brandColor)');
      } else {
        this.activeItemId = undefined;
      }
    }
